...
} from "../../../../../chunk-VAJUTR4O.mjs";
import {
  buildDesignSummary
} from "../../../../../chunk-MDVTGN5Y.mjs";
import {
  createGoogle,
  external_exports,
  generateText
...
function createManagedPool(options2) {
  const {
...
async function fetchPolyfill() {
  return typeof globalThis.fetch !== "undefined" ? globalThis.fetch : (await import("../../../../../lib-7QCB54Z2.mjs")).default;
}
...
__name(inflateHistoryVersion, "inflateHistoryVersion");
var Liveblocks = class {
   ...
   */
  async getVersionHistory(roomId, params = {}, options2) {
    const res = await this.#get(
   ...
   */
  async createVersionHistorySnapshot(roomId, options2) {
    const res = await this.#post(
      url`/v2/rooms/${roomId}/versions`,
      {},
      options2
   ...
   */
  async getYjsVersion(params, options2) {
   ...
   * @param options.signal (optional) An abort signal to cancel the request.
   */
  async deleteVersion(params, options2) {
    const { roomId, versionId } = params;
    const res = await this.#delete(
      ...
      const toolsConfig = {
        addNode: {
          ...
          inputSchema: external_exports.object({
            id: external_exports.string().describe("A unique ID for the new node. Typically shape-timestamp-random."),
            label: external_exports.string().describe("The label/title of the node."),
            shape: external_exports.enum(["rectangle", "diamond", "circle", "pill", "cylinder", "hexagon"]),
            color: external_exports.enum(["gray", "blue", "purple", "orange", "red", "yellow", "green", "teal"]),
        ...
        moveNode: {
          ...
          inputSchema: external_exports.object({
            id: external_exports.string().describe("The ID of the node to move."),
            position: external_exports.object({
              x: external_exports.number(),
              y: external_exports.number()
            })
        ...
        resizeNode: {
          ...
          inputSchema: external_exports.object({
            id: external_exports.string().describe("The ID of the node to resize."),
        ...
        updateNodeData: {
          ...
          inputSchema: external_exports.object({
            id: external_exports.string().describe("The ID of the node to update."),
            label: external_exports.string().optional(),
            color: external_exports.enum(["gray", "blue", "purple", "orange", "red", "yellow", "green", "teal"]).optional(),
        ...
        deleteNode: {
          ...
          inputSchema: external_exports.object({
            id: external_exports.string().describe("The ID of the node to delete.")
          })
        },
        addEdge: {
          description: "Add a connection (edge) between two nodes.",
          inputSchema: external_exports.object({
            id: external_exports.string().describe("A unique ID for the new edge. Typically edge-timestamp-random."),
            source: external_exports.string().describe("The source node ID."),
            target: external_exports.string().describe("The target node ID."),
            label: external_exports.string().optional().describe("Optional label for the edge.")
          })
        },
        deleteEdge: {
          description: "Delete an existing edge.",
          inputSchema: external_exports.object({
            id: external_exports.string().describe("The ID of the edge to delete.")
          })
        }
      };
...
- IMPORTANT — Edges: ALWAYS call addEdge after you have added the relevant nodes. The "source" and "target" fields in addEdge MUST exactly match the "id" values you used in addNode calls. Always add a short descriptive label to every edge specifying the protocol or action (e.g., "HTTPS Request", "Verify Token", "Cache Read", "Publish Order Event", "Consume Job", "SQL Write"). Do NOT leave edge labels empty.`;
      ...
      for (let i = 0; i < toolCalls.length; i++) {
        ...
        const actionPayload = toolCall.input;
        ...
        try {
          await liveblocks.mutateStorage(roomId, ({ root }) => {
            ...
            if (action === "addNode") {
              ...
              const size = SHAPE_DEFAULTS[shape] || { width: 120, height: 60 };
              const newNode = {
                id: actionPayload.id,
                type: "canvasNode",
                position: {
              ...
              if (node) {
                ...
                if (data && typeof data.set === "function") {
                  if (actionPayload.label !== void 0) data.set("label", actionPayload.label);
                  if (actionPayload.color !== void 0) data.set("color", actionPayload.color);
                  if (actionPayload.shape !== void 0) data.set("shape", actionPayload.shape);
                } else if (data) {
                  const newData = { ...data };
                  if (actionPayload.label !== void 0) newData.label = actionPayload.label;
                  if (actionPayload.color !== void 0) newData.color = actionPayload.color;
                  if (actionPayload.shape !== void 0) newData.shape = actionPayload.shape;
